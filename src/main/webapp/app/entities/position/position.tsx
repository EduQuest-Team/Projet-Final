  ...
  return (
    <div>
      <h2 id="position-heading" data-cy="PositionHeading">
        <Translate contentKey="pharmaAiApp.position.home.title">Positions</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="pharmaAiApp.position.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/position/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="pharmaAiApp.position.home.createLabel">Create new Position</Translate>
          </Link>
        </div>
      </h2>
        ...
        {positionList && positionList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="pharmaAiApp.position.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                ...
                <th>
                  <Translate contentKey="pharmaAiApp.position.pharmacie">Pharmacie</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {positionList.map((position, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{position.pharmacie ? <Link to={`/pharmacie/${position.pharmacie.id}`}>{position.pharmacie.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="pharmaAiApp.position.home.notFound">No Positions found</Translate>
            </div>
          )
        )}
      </div>
