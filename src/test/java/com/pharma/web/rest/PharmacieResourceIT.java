...
class PharmacieResourceIT {
    ...
    void getAllPharmaciesWithEagerRelationshipsIsEnabled() throws Exception {
        when(pharmacieRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPharmacieMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(pharmacieRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }
    ...
    void getAllPharmaciesWithEagerRelationshipsIsNotEnabled() throws Exception {
        ...
        restPharmacieMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
    ...
    void getNonExistingPharmacie() throws Exception {
        // Get the pharmacie
        restPharmacieMockMvc.perform(get(ENTITY_API_URL_ID, Long.MAX_VALUE)).andExpect(status().isNotFound());
    }
    ...
    void putExistingPharmacie() throws Exception {
        ...
        Pharmacie updatedPharmacie = pharmacieRepository.findById(pharmacie.getId()).orElseThrow();
    ...
    void deletePharmacie() throws Exception {
        ...
        restPharmacieMockMvc
            .perform(delete(ENTITY_API_URL_ID, pharmacie.getId()).accept(MediaType.APPLICATION_JSON))
